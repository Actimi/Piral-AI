...
async function isMonorepoPackageRef(refName, root) {
    const [monorepoRoot, client] = await detectMonorepoRoot(root);
    if (monorepoRoot) {
        const c = npm_clients_1.clients[client];
...
async function uninstallNpmPackage(client, packageRef, target = '.', ...flags) {
    try {
        const { uninstallPackage } = npm_clients_1.clients[client.direct];
...
async function installNpmPackage(client, packageRef, target = '.', ...flags) {
    try {
        const { installPackage } = npm_clients_1.clients[client.direct];
 ...
 */
async function dissectPackageName(baseDir, fullName) {
