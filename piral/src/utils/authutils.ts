...
export function getErrorsForInput(
    ...
    return outcome?.issue
        ?.filter((issue) =>
            isExpressionMatch(issue.expression?.[0], expression)
        )
...
export function getIssuesForExpression(
    ...
    return outcome?.issue?.filter((issue) =>
        isExpressionMatch(issue.expression?.[0], expression)
    )
}

const ARRAY_INDEX_REGEX = /\[\d+\]/
function isExpressionMatch(
    ...
    const isExpr1Indexed =
        typeof expr1 === "string" && ARRAY_INDEX_REGEX.test(expr1)
    const isExpr2Indexed =
        typeof expr2 === "string" && ARRAY_INDEX_REGEX.test(expr2)
