  ...
  "src/input/node.ts"() {
    ...
    DeclVisitor = class {
      ...
      getInterface(node) {
        ...
        const type = checker.getTypeAtLocation(node);
        const decls = type.symbol.declarations.filter(ts4.isInterfaceDeclaration).filter((m) => !isImportedFile(m, root2, availableImports));
        const clauses = [];
        const props = [];
...
// src/commands.ts
function setupVisitorContext(name, root2, files, imports, log, flags) {
    ...
    if (ps) {
      ...
      values.forEach((value) => {
        if (typeof value === "string") {
          const path = (0, import_path7.resolve)(typingsPath, "..", value);
...
async function generateDeclaration(options) {
  ...
  const sources = [
    ...files.map((file) => findAppRoot(root2, file)),
    ...apis.map((api) => findAppRoot(root2, api.file)),
    ...types.map((type) => findAppRoot(root2, type))
  ];
...
async function retrieveTypings(options) {
  ...
  const log = wrapLogger(logger, logLevel);
  log.verbose(`Aggregating the sources from "${root2}".`);
  const sources = [...files.map((file) => findAppRoot(root2, file)), ...types.map((type) => findAppRoot(root2, type))];
  log.verbose(`Setting up a visitor context for "${name}".`);
  const context = setupVisitorContext(name, root2, sources, imports, log, {
    noIgnore,
    noModuleDeclaration
...
var ts5, import_path7;
var init_commands = __esm({
  "src/commands.ts"() {
    ts5 = __toESM(require("typescript"));
  ...
  return {
    ...
    async "before-init"(context) {
      const refs = await retrieveTypings({
        root: (0, import_path8.dirname)(originalFile),
        types: [originalFile],
        plugins: [],
        logger: context.log,
    ...
    this.rasterize(row).forEach((rrow, r) => {
      ...
      rrow.forEach((col, c) => {
        ...
        if (wrapWidth > mixin.stringWidth(col)) {
          ts6 += " ".repeat(wrapWidth - mixin.stringWidth(col));
        }
        if (row[c].align && row[c].align !== "left" && this.wrap) {
          const fn = align[row[c].align];
          ts6 = fn(ts6, wrapWidth);
          if (mixin.stringWidth(ts6) < wrapWidth) {
            ts6 += " ".repeat((width || 0) - mixin.stringWidth(ts6) - 1);
          }
        }
...
function addBorder(col, ts6, style) {
  if (col.border) {
    if (/[.']-+[.']/.test(ts6)) {
      return "";
    }
    if (ts6.trim().length !== 0) {
      return style;
    }
    return "  ";
  }
...
function writeFile2(path, content) {
  const full = (0, import_path9.resolve)(root, path);
  const dir = (0, import_path9.dirname)(full);
  if (!(0, import_fs5.existsSync)(dir)) {
    (0, import_fs5.mkdirSync)(dir, { recursive: true });
  }
  (0, import_fs5.writeFileSync)(full, content, "utf8");
}
function runScript(script, cwd) {
  return new Promise((resolve8, reject) => {
    const cp = (0, import_child_process.exec)(script, { cwd });
    cp.on("close", (code, signal) => code === 0 ? resolve8() : reject(new Error(signal)));
  });
}
async function runCli() {
  ...
  const { generateDeclaration: generateDeclaration2 } = (init_index(), __toCommonJS(index_exports));
...
(async () => {
  try {
    ...
    await runScript("npm install typescript@^5", (0, import_path9.resolve)(__dirname, ".."));
  }
  try {
    await runCli();
  } catch {
